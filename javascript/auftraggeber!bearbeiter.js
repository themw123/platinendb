...
function getAuftraggeber(response) {
  ...
  for (var i = 0; i < response.length; i++) {
    ...
    if (admin != 1) {
      var op1 =
        "<option value='" + namee + "' " + selected + ">" + namee + "</option>";
...
$("#add1").on("click", function () {
  ...
  if (auftr.length > 0 && !(lehr == null || lehr == "")) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (zustand == "erfolgreich") {
          ...
          document.getElementById("fehleraddbenutzer").innerHTML =
    ...
    if (auftr == null || auftr == "") {
      document.getElementById("fehleraddbenutzer").innerHTML =
        "Bitte gib einen Auftraggebernamen ein";
      $("#fehleraddbenutzer").show();
    } else if (lehr == null || lehr == "") {
      document.getElementById("fehleraddbenutzer").innerHTML =
        "Bitte wähle einen Lehrstuhl aus";
      $("#fehleraddbenutzer").show();
    }
  }
});
      ...
      success: function (data) {
        ...
        if (zustand == "erfolgreich") {
          ...
          if (zustand == "nichtadmin") {
            document.getElementById("fehleraddbenutzer").innerHTML =
              "Ein Admin kann nicht gelöscht werden.";
          } else {
            if (error.indexOf("foreign") >= 0) {
              document.getElementById("fehleraddbenutzer").innerHTML =
                "Der Auftraggeber ist bereits einer Platine zugewiesen.";
            } else {
              document.getElementById("fehleraddbenutzer").innerHTML =
                "Datenbankfehler: " + error;
            }
