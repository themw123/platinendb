    ...
    private function registerNewUserEst()
    {
        if (empty($_POST['user_name'])) {
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                $stmt->bind_param("i", $user_lehrstuhl);
                $stmt->execute();
                $user_lehrstuhl = $stmt->get_result()->fetch_assoc();
                ...
                if ($query_check_user_name->num_rows == 1) {
                    ...
                    $stmt = $this->db_connection->prepare(
                        "INSERT INTO users (user_name, admin, user_password_hash, user_email, lehrstuhl)
                        VALUES(?, 0, ?, ?, ?);"
                    );
                    $stmt->bind_param("sssi", $user_name, $user_password, $user_email, $user_lehrstuhl);
                    $stmt->execute();
